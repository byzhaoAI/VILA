...
class RandomFourierFeatures(nn.Module):
    def __init__(self, in_dim, k=1024, sigma=1.0, learnable=False):
        """
        in_dim: 原始 feature 维 D (X_fe 的列)
        k: 投影个数，映射后维度为 2*k
        sigma: 频率（核尺度）参数，W ~ N(0, 1/sigma^2)
        learnable: 是否把 W 设为可训练（可选）
        """
        super().__init__()
        self.k = k
        self.in_dim = in_dim
        # 初始化
        W = torch.randn(k, in_dim) / sigma  # N(0, 1/sigma^2)
        self.W = nn.Parameter(W, requires_grad=learnable)
        # 可用偏置
        self.b = nn.Parameter(2 * torch.pi * torch.rand(k), requires_grad=False)

    def forward(self, x):
        # x: (B, D)
        proj = F.linear(x, self.W) + self.b  # (B, k)
        return torch.cat([torch.sin(proj), torch.cos(proj)], dim=1)  # (B, 2k)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, global_pool=False, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        if self.use_block_weight:
            ...
            nn.init.xavier_uniform_(self.block_weight, gain=nn.init.calculate_gain('relu'))
            # nn.init.kaiming_uniform_(self.block_weight, nonlinearity='relu')
    ...
    def _add_rff_layer(self, rff_k, rff_sigma):
        self.rff = RandomFourierFeatures(self.num_features, k=rff_k, sigma=rff_sigma)
