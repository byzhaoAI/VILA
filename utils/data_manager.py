    ...
    def get_dataset(
        ...
        if self.args['model_name'] in ['vila']:
            if mode == "train":
                ...
                clip_trsf = transforms.Compose([*self._clip_test_trsf, transforms.RandomHorizontalFlip(p=1.0), *self._clip_common_trsf,])
            elif mode == "test":
                clip_trsf = transforms.Compose([*self._clip_test_trsf, *self._clip_common_trsf])
            else:
                raise ValueError("Unknown mode {}.".format(mode))
            
            if ret_data:
                return data, targets, CLIPDummyDataset(data, targets, trsf, clip_trsf, self.use_path)
            else:
                return CLIPDummyDataset(data, targets, trsf, clip_trsf, self.use_path)
...
class CLIPDummyDataset(Dataset):
    def __init__(self, images, labels, trsf, clip_trsf, use_path=False):
        assert len(images) == len(labels), "Data size error!"
        self.images = images
        self.labels = labels
        self.trsf = trsf
    ...
    def __getitem__(self, idx):
        if self.use_path:
            image_data = pil_loader(self.images[idx])
        else:
            image_data = Image.fromarray(self.images[idx])
        label = self.labels[idx]

        return idx, self.trsf(image_data), self.clip_trsf(image_data), label
