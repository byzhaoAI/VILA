    ...
    if 'vila' in args["model_name"]:
        if name == "pretrained_vit_b16_224" or name == "vit_base_patch16_224":
            ...
            if name == "vit_base_patch16_224_lora":
                model = vit_lora.vit_base_patch16_224_cllora(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            elif name == "vit_base_patch16_224_in21k_lora":
                model = vit_lora.vit_base_patch16_224_in21k_cllora(num_classes=0,
                    global_pool=False, drop_path_rate=0.0, tuning_config=tuning_config)
                model.out_dim=768
            else:
                raise NotImplementedError("Unknown type {}".format(name))
            ...
            if name == 'laion400m_e32_clip':
                ...
                _pretrained = 'laion2b_s34b_b88k'
            elif name == 'openai_clip':
                _pretrained = 'openai'
            else:
                raise NotImplementedError("Unknown type {}".format(name))
